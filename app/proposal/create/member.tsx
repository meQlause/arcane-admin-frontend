...
export default function ProposalCreateMember({ rdt }: any) {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const options = {
      ...
      body: `{"name":"arcane","content": {"title": "${title}", "description": "${description.trim()}", "picture": "${res1.slice(39,)}", "endEpoch": "${votingDuration}"}, "metadata":{"picture": "${res1.slice(39,)}", "endEpoch": "${votingDuration}"}}`
    };
