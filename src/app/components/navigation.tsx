...
const Icon = ({ path }: { path: string }) => {
  return <Image src={path} alt="icon" width={30} height={30} priority />;
};
...
export function Navigation({ links, socials }: any) {
  ...
  return (
    <>
      <ul
        className={`px-6 w-full flex flex-col gap-3 h-[calc(100vh-68px-env(safe-area-inset-bottom))] max-h-[calc(100vh-84px)] lg:h-screen lg:max-h-[calc(100vh-88px)] pb-4 ${
          !walletConnect && pathname.indexOf("/admin") < 0 ? "max-lg:pb-20" : ""
        } ${
          walletConnect && pathname.indexOf("/admin") > -1 ? "max-lg:pb-28" : ""
        } ${
          walletConnect && pathname.indexOf("/admin") < 0 ? "max-lg:pb-40" : ""
        } scroll-bg-white overflow-auto font-maven-pro`}
      >
        {pathname.indexOf("/admin") < 0 && (
          <li className="order-9 lg:order-1">
            <Card className="!bg-primary-100 !shadow-none max-lg:mb-8 max-lg:mt-4 lg:mb-4">
              <div className="flex lg:flex-col gap-5 lg:gap-3 lg:text-center mb-5 last:mb-0">
                ...
                />
                <div>
                  <h2 className="font-semibold text-lg lg:text-xl text-gray-800 mb-1 lg:mb-2">
                    Arcane Labyrinth
                  </h2>
              ...
              {!walletConnect && role === "" && (
                <>
                  ...
                  <Button
                    ...
                    variant={"primary"}
                    loading={"none"}
                    className="lg:!w-fit mx-auto lg:!px-8 lg:py-2.5"
              ...
              {walletConnect && role === RoleType.Unregistered && (
                <>
                  ...
                  <Popup
                    ...
                    backdropClose={true}
                    handleClose={handleClosePopupSignup}
                  >
                    <PopupBody>
                      <SignUp />
                      <Button
        ...
        {links.map((item: any, index: number) => {
          ...
          if (
            (isAdmin && item.name === "About") ||
            (!isAdmin && item.name === "Setting")
          ) {
            return null;
          }
        ...
        <li
          className="order-10 mt-auto z-[1] max-lg:bg-white max-lg:fixed max-lg:bottom-0 max-lg:pb-4 max-lg:w-[calc(100%-3rem)]"
              ...
              <>
                <hr
                  className={`border-t border-gray-300 my-4 ${
                    pathname.indexOf("/admin") > -1 ? "max-lg:mt-0" : ""
                  }`}
                />
                <button
                  type="button"
                ...
                >
                  ...
                  <div
                    className={`transition-all ${!loading ? "opacity-0" : ""}`}
                  >
                    <Image
                      src="/loading.svg"
                      alt="loading"
